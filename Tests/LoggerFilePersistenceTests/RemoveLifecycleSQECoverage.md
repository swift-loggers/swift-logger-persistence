...
| Rotated segment removal mechanics | LGP-6, LGP-9, LGP-27, LGP-39 | `FileLogStoreRemoveTests.swift` | Fully exported non-active rotated segments are unlinked; the active rotated segment preserves no accepted bytes before the removal boundary and remains writable for subsequent appends. Rotated segments with post-boundary bytes are compacted so the suffix survives byte-for-byte. Compaction never widens segment permissions past the writer-private (`0o600` / `0o700`) default — owner-class bits flow through the atomic rename, group and world bits are dropped from the replacement segment even if the pre-compaction boundary segment carried them. |
...
| Stale boundary fail-closed | LGP-2, LGP-9, LGP-24, LGP-25, LGP-39 | `FileLogStoreRemoveStaleBoundaryTests.swift`, `FileLogStoreCompactionStaleBoundaryTests.swift` | Missing boundary segment, symlink replacement, identity mismatch, segment size shorter than `exportedPrefixEnd`, post-validation disappearance or symlink swap, duplicate numeric rotated topology, and a vanish/symlink/identity/truncate landing in either the compaction-read window OR the final pre-replace window each fail as `.removalBoundaryStale` before mutating the affected entry. The compaction temporary is unwound and no `.swift-logger-compact-*` artifact remains. |
