...
| Export temporary confidentiality and permissions | LGP-8, LGP-24, LGP-25, LGP-32 | Covered | `FileLogStoreExportTests.swift`, `ByteStableExportSQECoverage.md` |
...
| --- | --- | --- |
| `OperationBoundary` | Lease-match release, stale lease rejection during hand-off, FIFO hand-off across multiple queued waiters, non-cancelable queued-waiter semantics, and bounded cleanup on failure paths. | `OperationBoundaryTests.swift` |
| `TestRendezvous` | Non-blocking async rendezvous, cancellation before and after waiter registration, multi-waiter broadcast for pause and release sides, timeout that does not join a non-cancellation-cooperative inner operation, and no `DispatchSemaphore.wait` inside actor-isolated seams. | `TestRendezvousTests.swift`, `FileLogStoreExportTests.swift`, `FileLogStoreRemoveConcurrencyTests.swift` |
| Export temporary containment | No readable regular export temp file is exposed in the destination parent during write phase; covered failure-seam paths observe private temp containment cleanup; final permissions follow platform umask semantics without process-wide `umask` mutation. | `FileLogStoreExportTests.swift`, `FileLogStoreExportDestinationTests.swift`, `ByteStableExportSQECoverage.md` |
